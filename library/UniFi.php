...
class UniFi
{
    ...
    function __construct($controller = null, $json = null)
    {
        if ($controller !== null) {
            ...
            $this->cookies = $blockCipher->decrypt($controller->getCookies());
    ...
    */
    public function login($force = false)
    {
        ...
        if ($this->cookies && $force == false) {
            if ($this->cookies != '/tmp/unifi') {
    ...
    */
    ...
    /*
    list site guest settings
    returns a array of the site configuration settings
    */
    public function list_settings_guest()
    {
        $return = array();
        if (!$this->is_loggedin) return $return;
        $return = array();
        $json = json_encode(array());
        $content = $this->exec_curl($this->baseurl . "/api/s/" . $this->site . "/rest/setting/guest_access", "json=" . $json);
        $content_decoded = json_decode($content);
        if (isset($content_decoded->meta->rc)) {
            if ($content_decoded->meta->rc == "ok") {
                if (is_array($content_decoded->data)) {
    ...
    */
    ...
    private function exec_curl($url, $data = "")
    {
        ...
        if(isset($contentDecoded['meta']['msg']))
        {
            $responseCode = $contentDecoded['meta']['msg'];
            if (!$this->failed > 0) {
                if($responseCode != 'api.err.LoginRequired') {
                    $this->failed++;
                    $this->login(true);
